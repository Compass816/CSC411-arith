...
impl<T: Clone> Array2<T> {
    pub fn trim_to_even_dimensions(&self) -> Array2<T> {
        let new_width = if self.width % 2 == 0 { self.width } else { self.width - 1 };
        let new_height = if self.height % 2 == 0 { self.height } else { self.height - 1 };

        let mut new_data = Vec::with_capacity(new_width * new_height);

        for y in 0..new_height {
            for x in 0..new_width {
                let element = self.get(x, y).clone();
